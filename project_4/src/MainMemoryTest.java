...
public class MainMemoryTest extends student.TestCase {
	...
	public void testMainMemory() {
		...
		assertEquals(test1.getBlockSize() == 1024, true);
		assertEquals(test2.getBlockFillSize() == 0, true);
		assertEquals(test3.getBuff().length == 4096, true);
		...
		test1.setInBuff(c);
		assertEquals(test1.getBlockSize() == 32, true);
		assertEquals(test2.getBlockFillSize() == 32, true);
		assertEquals(test3.getBuff().length == 32, true);
		assertEquals(test1.getInBuff().array().length == 32, true);
	}
